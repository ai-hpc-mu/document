...
myst:
  html_meta:
    "description lang=en": |
      Top-level documentation for Exscale Cluster Knowlege, with links to the rest
      of the site..
...
A clean, Bootstrap-based Sphinx theme by and for [Exascale Cluster Home Page](https://exascale.mahidol.ac.th).
...
Information about using Cluster Softwares.
...
```
## Applicaion
Rule and process for registration.
```{toctree}
:maxdepth: 2
...
Exascale clusters system consists of high performence computer(Nvidia A100 and H100 server, high speed network( The compute nodes are connected with HDR-200 InfiniBand). Those provisions Slurmsupercomputer cluster and kubernetes native cloud.
```{toctree}
:maxdepth: 2
